...
#define T_MIN -17.0f
#define T_MAX 17.0f
#define RS02_TARGET_POS_RAD 6.00f
...
static float PID_Update(PIDrs02_t *pid, float measure)
{
...
void RS02_UserInit(void)
{
    ...
    Set_RobStride_Motor_parameter(&motor1, 0x7005, Speed_control_mode, Set_mode);
    HAL_Delay(5); // 延时等待
    ...
    rs02_pos_pid.kp = 10.0f;
    rs02_pos_pid.ki = 0.05f;
    rs02_pos_pid.kd = 0.6f;

    rs02_pos_pid.target = motor1.Pos_Info.Angle + 3.1415926f;

    rs02_pos_pid.max_out = 0.5f; // 最大速度 rad/s（很关键）
    rs02_pos_pid.i_limit = 1.0f;
...
void RS02_Task(void)
{
    ...
    {
        rs02_last_cmd_tick = HAL_GetTick();

        float current_angle = motor1.Pos_Info.Angle;
        float speed_cmd = PID_Update(&rs02_pos_pid, current_angle);

        if (speed_cmd > RS02_LIMIT_SPEED_RAD_S)
            speed_cmd = RS02_LIMIT_SPEED_RAD_S;
        if (speed_cmd < -RS02_LIMIT_SPEED_RAD_S)
            speed_cmd = -RS02_LIMIT_SPEED_RAD_S;

        RobStride_Motor_Speed_control(&motor1, speed_cmd, RS02_LIMIT_CURRENT_A);
    }
}
// -------------------- 数据转换工具 --------------------
